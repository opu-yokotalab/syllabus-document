...
\section{本書について}
本書は2009年度のシラバスシステムのインストールマニュアルである。

\section{シラバスシステム動作環境}
シラバスシステムはLinuxオペレーティングシステムでの動作を想定している。
実際のシラバスシステムの運用はVine Linux4.2を用いている。
本書では
\begin{itemize}
\item Vine Linux
\item Fedora
...
aptを用いるにはroot(管理者)になる必要があります。ユーザーの切り替えはsuコマンドを用います。suとはswitch userの略です。
書式としてはsu $"ユーザー名"$です。ユーザー名を省略するとだいたいrootにswitchしようとします。\\
\begin{shadebox}
  \$ su\\
  パスワード(P):パスワードを入力\\
...
yumとは、Fedoraに標準でついているプログラムで、このプログラムを使うと
簡単にインストールやアンインストールができます。基本的にはaptコマンドと同じと考えていいです。\\
...
なお、外部からアクセスできるようにするためにはファイヤーウォールの設定でポート80番（HTTPプロトコル）を通すように設定しておきます。

\subsubsection*{Ubuntu系(MIE)などの場合}
apacheのAPACHE\_RUN\_USERがapacheではなくwww-dataなので、プロセスの実行者がapacheではなくwww-dataとなる。
したがって、シラバスシステム全体の所有者をwww-dataに変えるか、APACHE\_RUN\_USERをapacheに変更する。
インストールするコマンドは以下に示します。
...
Firefoxのアドレス欄にlocalhostと入力してみて下さい。It work!と表示されればapacheは動作しています。
...
\begin{shadebox}
  \# emacs /var/www/html/testphp.php
\end{shadebox}
...
VineLinuxの場合は/etc/apache2/conf/httpd.confに記述していきます。httpd.confの中にはディレクトリ毎の設定を記述する部分が以下のようにあります。ドキュメントルートでPHPを実行出来ればよいのでドキュメントルートに対しての設定部分に追記していきます。\\
